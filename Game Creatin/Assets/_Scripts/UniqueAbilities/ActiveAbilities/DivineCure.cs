...
public class DivineCure : MonoBehaviour, IActiveAbility
{
    ...
    [SerializeField]
    private float _cooldown, _HealProcent, _duration, _healRange;
    private float _width, _height, _cooldownConst, _durationConst, _fillAmountTime;
        ...
        if (_isReady)
        {
            ...
            else if (_isCast && Input.GetMouseButton(0))
            {
                ...
                if (hex != null && hex != _hexagonCast)
                {
                    ...
                    Physics2D.CircleCast(hex.position, _healRange, Vector2.zero, contactFilter2D, hits2D);
                    for (int i = 0; i < hits2D.Count; i++)
                    {
                        ...
                        if (!_listHexagonCast.Contains(hexagon.GetHexagonMain())
                            && (hex.position - hexagon.GetArrayElement().position).magnitude <= _healRange)
                        {
            ...
            else if (_isCast && Input.GetMouseButtonUp(0))
            {
                if (_listHexagonCast.Count > 0)
                {
                    ...
                    for (int i = 0; i < _listHexagonCast.Count; i++)
                    {
                        _listHexagonCast[i].Sprite.color = new Color(0, 0, 1, 1);
